...
| **边听边逛** | 最要紧的一条：歌放到一半你去首页搜别的歌、翻榜单、看资讯，**声音不会断**（见下面「两套播放源」） |
| **播放历史** | 真的播了才记，同一首只留最近一次，上限 300 条 |
| **搜索历史** | 记住搜索词，点一下直接搜，可一键清空 |
| **下载管理** | 走系统 DownloadManager：断点续传、通知栏进度、文件落到 `Music/歌曲宝/`，App 内有下载列表和进度；没播过的歌也能直接解析下载 |
| **通知栏 / 控制中心** | 封面 + 歌名 + 上一首/播放暂停/下一首，锁屏可用，耳机线控可用 |
| **后台播放** | 前台服务保活，切后台、锁屏、划走界面都不打断 || **迷你播放条** | 常驻底部的封面/歌名/进度条（可点进度条跳转）/收藏/上一首/下一首/更多菜单 |
...
网页里的 `<audio>` 是页面的子资源 —— 页面一换，它就没了。所以只要你在听歌时去翻页，歌就断。
解决办法是「接管线」：

| 什么时候 | 谁在响 |
|---|---|
| 你在歌曲页上（看得到歌词、进度） | **网页** 的 `<audio>` |
| 你从歌曲页跳走到了别的页面（首页、搜索、榜单…） | **原生 ExoPlayer** 接管，从当前进度接着响 |
| 你又回到正在放的那首歌的页面 | 网页重新接管，进度也交回给网页，不是从头再来 |

交接协议只有两个信号：

* 网页侧：注入脚本在「马上要出声」前喊一声 `GB.onWillPlay()` → 原生立刻让位并把进度记下；
* 原生侧：`WebViewClient.onPageStarted` 发现「正在放 + 新页面不是这首歌」→ 接管，
  用的是页面里已经解析好的那条直链（`<audio>` 的 `currentSrc`），所以不需要重新请求站点。

原生模式下切歌（下一首/上一首）也**不需要把用户的页面拽走**：
`GbResolver` 自己抓 `/music/{id}` 取出 `play_id`，再调站点自己的
`POST /member/common-play-url` 拿直链 —— 走的是站点公开流程，不需要登录。
...
2. **下载地址是「边播边解析」出来的**。站点要先点一次播放才会去拿直链。
   现在 App 自己会调站点的解析接口（`GbResolver`），所以没播过的歌也能直接下；
   失败时会退回「翻到歌曲页点下载歌曲」。注意站点对免费用户**有解析次数限制**，
   用完了 App 和网页都解析不出来（会明确提示原因）。
3. **站点改版会打断注入**：注入脚本靠 `#custom-audio-player`、`window.appData`、
   `#player-toggle-btn` 这几个锚点。锚点没了 → 迷你播放条不更新、连播失效，
   但网页本身照常能用。修法就是更新 `inject.js`。
4. **原生接管后，直链来源是站点解析出来的 CDN 地址**（实测无需 Referer，带了反而 403）。
   如果某条直链落到需要 Referer/Cookie 的站点自建地址上，代码里已按 host 分开带头处理；
   万一还是失败，会自动跳到队列里的下一首，而不是卡死。
...
目前只有一组用例，盯的是最容易写错、又最不容易在身边发现的地方：
把站点页面里那段被 JS 转义过的 `window.appData` 还原成真 JSON
（`app/src/test/resources/music4188.html` 是**真实抓下来的歌曲页**，
断言到「晴天 / 周杰伦 / 04:29 / 封面 http 直链」这一级）。
...
│   ├── App.kt
│   ├── core/
│   │   ├── Models.kt                # Song / Playlist / DownloadRec / AppSettings / NowPlaying / Source
│   │   ├── LocalStore.kt            # JSON 持久化（StateFlow + 原子写）
│   │   ├── PlayerHub.kt             # 播放中枢：网页 <-> 原生 的交接、播放队列
│   │   ├── GbResolver.kt            # 不用 WebView 也能拿直链：/music/{id} + common-play-url
│   │   └── DownloadHelper.kt        # DownloadManager 封装
│   ├── web/
│   │   ├── WebShell.kt              # WebView 配置、URL 分流、注入装配
│   │   └── JsBridge.kt              # @JavascriptInterface 桥（名字叫 GB）
│   ├── player/
│   │   ├── NativePlayer.kt          # ExoPlayer：网页翻走了之后接着响的那一个
│   │   └── PlaybackService.kt       # 前台服务 + MediaSessionCompat + 通知
│   └── ui/
